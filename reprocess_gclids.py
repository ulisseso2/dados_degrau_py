#!/usr/bin/env python3
"""
Script para reprocessar GCLIDs não encontrados

Este script permite executar o reprocessamento de GCLIDs não encontrados 
de forma independente do dashboard Streamlit.

Uso:
    python reprocess_gclids.py --period 30  # Reprocessa últimos 30 dias
    python reprocess_gclids.py --all        # Reprocessa todos os GCLIDs não encontrados
    python reprocess_gclids.py --count      # Apenas mostra quantos não foram encontrados
"""
...
def get_google_ads_client():
    ...
    try:
        ...
        # Carrega configurações do Google Ads
        ...
        print(f"Erro ao configurar cliente Google Ads: {e}")
        return None, None

def reprocess_gclids_batch(client, customer_id, gclid_list):
    """Reprocessa um lote de GCLIDs"""
    ...
    for i in range(0, len(gclids_list), batch_size):
        ...
        for date_str in date_ranges:
            ...
            try:
                ...
                print(f"Erro na consulta para {date_str}: {ex}")
                continue
...
def main():
    parser = argparse.ArgumentParser(description="Reprocessa GCLIDs não encontrados")
    ...
    if args.count:
        ...
        print(f"Total de GCLIDs não encontrados: {total}")
        return
    ...
    if args.all:
        ...
        print(f"Reprocessando {len(gclid_list)} GCLIDs não encontrados...")
    else:
        end_date = datetime.now().date()
        start_date = end_date - timedelta(days=args.period)
        gclid_list = get_gclids_by_date_range(start_date, end_date)
        print(f"Reprocessando {len(gclid_list)} GCLIDs não encontrados dos últimos {args.period} dias...")
    
    if not gclid_list:
        print("Nenhum GCLID não encontrado para reprocessar!")
        return
    
    # Configura cliente Google Ads
    client, customer_id = get_google_ads_client()
    if not client or not customer_id:
        print("Erro: Não foi possível configurar o cliente Google Ads")
        return
    ...
    for i in range(0, len(gclid_list), batch_size):
        ...
        print(f"Processando lote {i//batch_size + 1} ({len(batch)} GCLIDs)...")
