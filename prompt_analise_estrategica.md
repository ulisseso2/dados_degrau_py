# Prompt de Análise Estratégica — `analise_geral.py`

Arquivo único contendo:
- **SYSTEM_PROMPT**: papel, contexto de domínio, regras de raciocínio, formato de saída
- **USER_PROMPT_TEMPLATE**: estrutura do payload com placeholders
- **Notas de integração** para o Ulisses

O conteúdo dos blocos de código abaixo é o que vai parar dentro de `utils/prompts/analise_geral_prompt.py` como constantes Python.
...
O user prompt é montado dinamicamente pela página `analise_geral.py` a cada execução. Estrutura:
...
Cada `{tabela_*}` é uma string de tabela markdown gerada por uma query da pasta `queries/analise_geral/`. Recomendo um helper `format_table_for_prompt(df)` em `utils/` que recebe um DataFrame e devolve string markdown limitada a, no máximo, 50 linhas (com nota "*top 50 de N*" no rodapé quando truncado).

### Sobre `{amostra_*}`

São blocos de evidência qualitativa. Não passar transcrições inteiras (estoura contexto). Estratégia recomendada:
- Para "padrões de ligações vendidas": pegar 5 ligações vendidas no período, extrair as 3-5 melhorias e os 3 pontos fortes que já estão tabulados no MySQL pelo `transcricao_analyzer.py`, e concatenar.
- Para "erros recorrentes por vendedor": agregar os "erros mais caros" já produzidos pelo analyzer, agrupados por vendedor, top 3 por frequência.

Isso aproveita a infraestrutura existente em vez de mandar texto bruto pra IA refazer trabalho.

### Sobre o `{relatorio_ia_marketing_consolidado}`

O `relatorios_ia.py` já gera um relatório. Não regerar — pegar o mais recente do período via query e injetar como contexto.

### Sobre o modelo e tokens

- Modelo recomendado: `claude-opus-4-7` (raciocínio mais robusto justifica o custo nesta análise pontual)
- `max_tokens`: 8000 (relatório esperado entre 1.500 e 3.500 palavras + autoavaliação)
- `temperature`: 0.3 (análise factual, não criativa)
- Estimativa de tokens de entrada: 30.000 a 80.000 dependendo do período. Validar antes de executar para evitar surpresa de custo.

### Sobre validação pós-execução

Adicionar na página um botão "Validar uso dos dados" que dispara uma segunda chamada ao Claude com o relatório gerado e o payload original, perguntando: "Quais blocos de dados do payload não foram utilizados ou foram subutilizados no relatório?". Resposta humana valida se o prompt está pegando tudo.

### Sobre persistência

Cada relatório gerado deve ser salvo no MySQL com (empresa, data_inicio, data_fim, payload, response, modelo, tokens_in, tokens_out, custo_estimado). Permite comparar relatórios mês a mês e detectar drift.

### Sobre custo recorrente

Se a análise for rodada mensalmente, criar um histórico de tokens consumidos. Em volume de payload alto, considerar Batch API (50% de desconto) — só não usar Batch se a expectativa for resposta no mesmo dia.
