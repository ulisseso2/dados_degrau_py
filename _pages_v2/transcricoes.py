...
def _contexto_da_row(row_data) -> dict:
    """Monta o contexto canônico (tipo de ligação, empresa, etapa, P1/P2)
    a partir da linha do DataFrame. Colunas de qualificação são opcionais —
    aparecem quando o SQL da página expõe (ver IMPLEMENTACAO_ULISSES.md §SQL)."""
    def _g(campo):
        try:
            v = row_data.get(campo)
        except Exception:
            return None
        if v is None:
            return None
        txt = str(v).strip()
        return txt if txt and txt.lower() not in ('nan', 'none', 'nat') else None

    def _raw(campo):
        try:
            return row_data.get(campo)
        except Exception:
            return None

    return montar_contexto_qualificacao(
        p1_pontos=_raw('p1_pontos'),
        p2_pontos=_raw('p2_pontos'),
        score_total=_raw('score_bot_total'),
        etapa_crm=_g('etapa'),
        tipo_ligacao=_g('tipo_ligacao'),
        empresa=_g('empresa'),
    )


def _crm_sync_ligacao(tid, row_data, analise, transcricao):
    """Envia transcrição + inteligência do LEAD ao CRM (nunca a avaliação do
    vendedor — ver política em utils/crm_sync_writer.py). Ativo apenas com
    CRM_SYNC_ENABLED=1 e oportunidade vinculada."""
    if not crm_sync_habilitado():
        return
    try:
        op_id = row_data.get('oportunidade_id')
    except Exception:
        op_id = None
    try:
        op_id = int(op_id) if op_id is not None and str(op_id).strip() not in ('', 'nan', 'None') else None
    except (TypeError, ValueError):
        op_id = None
    if not op_id or not analise.get('deve_avaliar'):
        return
    try:
        ai_eval = json.loads(analise.get('avaliacao_completa') or '{}')
    except (TypeError, ValueError):
        ai_eval = {}
    try:
        _payload = montar_payload_crm(
            ...
            data_evento=str(row_data.get('data_ligacao')) if row_data.get('data_ligacao') is not None else None,
            agente=str(row_data.get('agente')) if row_data.get('agente') is not None else None,
        )
...
def _executar_avaliacoes(df_base: pd.DataFrame, ids_selecionados: list):
    ...
    def _avaliar_uma(task):
        ...
        try:
            analise = ia.analisar_transcricao(tx, contexto_adicional=_contexto_da_row(row_data))
            if analise.get('erro'):
            ...
            if ok:
                _crm_sync_ligacao(tid, row_data, analise, tx)
                ...
                if st.button("🔁 Reavaliar esta ligação", key=f"reav_{tid}"):
                    ...
                    if not tx:
                        ...
                        with st.spinner("Reavaliando..."):
                            ia = TranscricaoAnalyzer()
                            analise = ia.analisar_transcricao(tx, contexto_adicional=_contexto_da_row(row_av))
                        if analise.get('erro'):
