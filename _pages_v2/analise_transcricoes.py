...
from utils.analise_helpers import _cor_nota, _safe_pct, _top_items, _gerar_html_relatorio, _PAT_CAT, _strip_cat, _extract_cat
from utils.cats_vendedor import _CATS_VENDEDOR, _CATS_LEGACY
from utils.qualificacao_dashboard import render_tab_bot_vs_ia
...
def _carregar_dados() -> pd.DataFrame:
    ...
    # ── Notas por categoria (0-100% do peso) — paridade com analise_chats ──
    def _extract_notas_pct(j):
        if not isinstance(j, dict):
            return {}
        notas = (j.get('avaliacao_vendedor') or {}).get('notas_por_categoria') or {}
    ...
    df['notas_pct'] = insight.apply(_extract_notas_pct)
    df['tlq_status'] = insight.apply(
        lambda x: (((x.get('avaliacao_vendedor') or {}).get('tratamento_lead_qualificado') or {}).get('status', '')
...
def run_page():
    ...
    df_base['tipo_ligacao'] = df_base['tipo_ligacao'].fillna('Sem triagem')
    if tipo_sel:
    ...
    with tab7:
        if df_av.empty:
            ...
            render_tab_bot_vs_ia(df_av, id_col='transcricao_id', canal_label='Ligações')
    ...
    with tab4:
        if df_av.empty:
            ...
            # ── Notas médias por categoria (time) — notas REAIS da IA ────────
            st.markdown("#### 📊 Notas Médias por Categoria — Time")
            cat_medias = {}
            for cat_key, (cat_label, _) in _CATS_VENDEDOR.items():
                vals = [nd[cat_key] for nd in df_av.get('notas_pct', pd.Series(dtype=object))
            ...
            if cat_medias:
                ...
                fig_cats.update_layout(height=340, margin=dict(t=10, b=10),
                                       showlegend=False, xaxis=dict(range=[0, 115]))
                st.plotly_chart(fig_cats, use_container_width=True)
            else:
                st.info("Nenhuma nota por categoria encontrada nos JSONs de avaliação.")

            # Radar de Competências por agente — notas REAIS por categoria
            # (substitui a antiga proxy fortes/(fortes+melhorias), que era
            # estruturalmente enviesada: o prompt sempre gera 3+3 itens)
            st.markdown("#### 🕸️ Radar de Competências por Agente")
            st.caption("Baseado nas notas por categoria atribuídas pela IA (0-100% do peso máximo).")
            top5_ag = df_av['agente'].value_counts().index.tolist()[:7]
            cats_keys = list(_CATS_VENDEDOR.keys())
            cats_lbl = [_CATS_VENDEDOR[k][0] for k in cats_keys]
            fig_radar = go.Figure()
            for ag in top5_ag:
                df_ag_r = df_av[df_av['agente'] == ag]
                scores = []
                for cat_key in cats_keys:
                    vals = [nd[cat_key] for nd in df_ag_r.get('notas_pct', pd.Series(dtype=object))
                            if isinstance(nd, dict) and cat_key in nd]
    ...
    with tab5:
        if df_av.empty:
            ...
            _cols_tab = [
                ...
                'etapa', 'vendedor_disclaimer', 'lead_disclaimer',
            ]
            # campos adicionais
            extras = []
        ...
        if df_base.empty:
            ...
            if sel_id:
                ...
                try:
                    ...
                    j = {}
                detalhe_t = carregar_detalhe_transcricao(sel_id)
                transcript = detalhe_t.get('transcricao', '')
