...
from utils.analise_helpers import _safe_pct, _cor_nota, _top_items, _gerar_html_relatorio
...
def _carregar_dados() -> pd.DataFrame:
    ...
    df['tlq_status'] = df['parsed_json'].apply(
        lambda j: (((j.get('avaliacao_vendedor') or {}).get('tratamento_lead_qualificado') or {}).get('status', '')
                   if isinstance(j, dict) else '')
    )
...
def run_page():
    ...
    # ABAS
    # ════════════════════════════════════════════
    tab1, tab_inteligencia, tab2, tab3, tab4, tab5, tab6 = st.tabs([
        "📈 Visão Geral",
    ...
    with tab6:
        if df_av.empty:
            ...
            render_tab_bot_vs_ia(df_av, id_col='chat_id', canal_label='WhatsApp')
