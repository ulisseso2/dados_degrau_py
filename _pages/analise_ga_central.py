...
def reprocess_not_found_gclids(client, customer_id, start_date, end_date, force_all=False):
    """
    Reprocessa GCLIDs marcados como 'Não encontrado' tentando encontrá-los novamente na Central.
    ...
    if force_all:
        ...
        st.info(f"Reprocessando {len(not_found_list)} GCLIDs não encontrados na Central...")
    else:
        not_found_list = get_gclids_by_date_range(start_date, end_date)
        st.info(f"Reprocessando {len(not_found_list)} GCLIDs não encontrados no período selecionado (Central)...")
    
    if not not_found_list:
        st.success("Não há GCLIDs não encontrados para reprocessar na Central!")
    ...
    try:
        ...
        if results:
            ...
            st.success(f"✅ {success_count} GCLIDs foram encontrados e atualizados na Central!")
        
        if still_not_found > 0:
            st.warning(f"⚠️ {still_not_found} GCLIDs ainda não foram encontrados na Central.")
...
def run_page():
    ...
    # --- SEÇÃO: REPROCESSAMENTO DE GCLIDs NÃO ENCONTRADOS (CENTRAL) ---
    st.header("🔄 Reprocessamento de GCLIDs (Central)")
    ...
    with col1:
        st.metric("Total de GCLIDs Não Encontrados (Central)", total_not_found)
    with col2:
        st.metric("Não Encontrados no Período (Central)", len(period_not_found))
    
    if total_not_found > 0:
        st.info("Os GCLIDs marcados como 'Não encontrado' na Central podem ter sido processados em momentos diferentes ou podem ter se tornado disponíveis na API do Google Ads.")
        
        col1, col2 = st.columns(2)
        
        with col1:
            if st.button("🔄 Reprocessar Período Atual (Central)", help="Reprocessa apenas os GCLIDs não encontrados no período selecionado"):
                if gads_client and customer_id:
                    with st.spinner("Reprocessando GCLIDs do período na Central..."):
        ...
        with col2:
            if st.button("🔄 Reprocessar Todos (Central)", help="Reprocessa todos os GCLIDs não encontrados na Central (pode demorar)"):
                if gads_client and customer_id:
                    with st.spinner("Reprocessando todos os GCLIDs não encontrados na Central..."):
        ...
        st.success("🎉 Não há GCLIDs marcados como 'Não encontrado' na Central!")
