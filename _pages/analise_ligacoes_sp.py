...
def _carregar_dados() -> pd.DataFrame:
    ...
    df['evaluation_ia'] = df['evaluation_ia'].where(df['evaluation_ia'] > 0)
...
# ──────────────────────────────────────────────
def run_page():
    st.title("📊 Análise de Ligações SP")
    st.caption(
        "Painel comercial · Análise de transcrições e avaliações — Central"
    )
    ...
    agente_sel = st.sidebar.multiselect(
        "Agente:", agentes_disp, default=agentes_disp, key="anl_sp_agente"
    )
    ...
    tipo_sel = st.sidebar.multiselect(
        "Tipo de ligação:", tipos_disp, default=tipos_disp, key="anl_sp_tipo"
    )
    ...
    with tab2:
        if df_av.empty:
            ...
            min_lig = st.slider(
                "Mínimo de ligações avaliadas:", 1, max(max_lig, 1), 1, key="min_lig_rank_sp"
            )
    ...
    with tab4:
        if df_av.empty:
            ...
            agente_rel  = st.selectbox(
                "Selecione o vendedor:", agentes_ord, key="anl_sp_agente_rel"
            )
