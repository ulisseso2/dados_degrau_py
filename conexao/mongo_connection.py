...
class JsonDataConnection:
    ...
    def __init__(self):
        ...
        self.json_file_path = "/home/ulisses/dados_degrau_py/consultas/eduqc.subjects_topics_summary.json"
        self._data = None
        ...
        try:
            ...
            for item in self._data:
                if item.get("subject") == subject_name:
                    ...
                    # Retorna com o nome normalizado
                    result = item.copy()
                    result["name"] = item.get("subject", "")
                    return result
    ...
    def get_all_subjects_with_topics(self) -> List[Dict[str, Any]]:
        ...
        try:
            ...
            # Normaliza os dados para incluir 'name'
            normalized_data = []
            for item in self._data:
                normalized_item = item.copy()
                normalized_item["name"] = item.get("subject", "")
                normalized_data.append(normalized_item)
            
            logger.info(f"Encontrados {len(normalized_data)} subjects com topics")
